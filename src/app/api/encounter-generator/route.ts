...
export async function POST(request: NextRequest) {
  try {
    ...
    const { name, enemyType, difficulty, environment, partySize, customPrompt, campaignContext } = body
    ...
    const response = await openRouterQueue.add(async () => {
      ...
      // Create the prompt for encounter generation
      let prompt = ''
      ...
      if (customPrompt) {
        ...
        let enhancedPrompt = `Create a detailed D&D 5e encounter for a campaign with:
...
Encounter Requirements: ${customPrompt}`
        
        // Add advanced inputs if provided
        const advancedInputs = []
        if (name) advancedInputs.push(`named ${name}`)
        if (enemyType) advancedInputs.push(`enemy type: ${enemyType}`)
        if (difficulty) advancedInputs.push(`difficulty: ${difficulty}`)
        if (environment) advancedInputs.push(`environment: ${environment}`)
        if (partySize) advancedInputs.push(`party size: ${partySize}`)
...
IMPORTANT: This encounter should fit into the campaign's narrative and connect to other elements like NPCs, locations, and the main plot.
...
Please return ONLY a valid JSON object with this EXACT structure:
{
  ...
  "enemies": "Detailed enemy description",
  "environment": "Environment description",
  "objectives": ["Objective 1", "Objective 2"],
  "rewards": "Detailed rewards description",
  "terrain": "Terrain features and obstacles",
  "hazards": "Environmental hazards",
  "reinforcements": "Potential reinforcements",
  "levelRange": "Recommended character levels",
  "estimatedDuration": "How long the encounter should take",
  "tactics": "Enemy tactics and behavior",
  "connection": "How this encounter connects to the campaign's story and other elements"
}`
      } else {
        prompt = `Create a detailed D&D 5e encounter with the following parameters:
- Enemy Type: ${enemyType || 'Mixed'}
- Difficulty: ${difficulty || 'Medium'}
- Environment: ${environment || 'Dungeon'}
- Party Size: ${partySize || '4-6 players'}
${name ? `- Name: ${name}` : ''}
...
Please return ONLY a valid JSON object with this EXACT structure:
{
  ...
  "tactics": "Enemy tactics and behavior"
      ...
      const apiResponse = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are a D&D 5e encounter creation expert. You must respond with ONLY valid JSON objects, no other text or explanations. Always format your response as a complete JSON object.'
            },
            {
              role: 'user',
              content: prompt
      ...
      try {
        ...
        try {
          encounter = JSON.parse(content.trim())
        } catch (parseError) {
          // If that fails, try to extract JSON from the content
          const jsonMatch = content.match(/\{[\s\S]*\}/)
          if (!jsonMatch) {
            console.error('No JSON found in content:', content)
            throw new Error('Failed to parse encounter data from AI response')
          }
          ...
          try {
            ...
            try {
              encounter = JSON.parse(fixedJson)
            } catch (thirdError) {
              console.error('Third JSON parse error:', thirdError)
              console.error('Attempted to parse:', fixedJson)
              throw new Error('Failed to parse encounter data. Please try again.')
            }
          }
        }
      } catch (error) {
        console.error('JSON extraction error:', error)
        throw new Error('Failed to extract encounter data from AI response')
      }
    ...
    // Auto-save the generated encounter
    try {
      ...
      console.log('Encounter auto-saved successfully')
    } catch (autoSaveError) {
      console.error('Auto-save error:', autoSaveError)
      // Don't fail the generation if auto-save fails
    }

    console.log('Encounter generated successfully using OpenRouter queue')
    return NextResponse.json({ encounter: response })
  } catch (error) {
